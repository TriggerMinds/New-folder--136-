...
class SourceCategory(str, enum.Enum):
    ...
    USER_UPLOAD_ARCHIVE = "user_upload_archive"
    RAW_LEAK_ARCHIVE = "raw_leak_archive"
    RESEARCHER_SITE = "independent_researcher"
    MIRROR_INDEX = "mirror_index"
    SEARCH_ENGINE = "repository_search"
...
class Source(Base):
    ...
    source_layer: Mapped[str] = mapped_column(String(20), nullable=False, default=SourceLayer.REFERENCE_ONLY.value)
    source_role: Mapped[str] = mapped_column(String(30), nullable=False, default=SourceRole.SIGNAL.value)
    source_category: Mapped[str] = mapped_column(String(30), nullable=False, default=SourceCategory.SPECIALIST_BLOG.value)
    can_create_primary_claim: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    can_create_artifact_discovery: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    can_create_reference_observation: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    discovery_priority: Mapped[str] = mapped_column(String(10), nullable=False, default=DiscoveryPriority.SECONDARY.value)
    ...
    present_in_country_pack: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    disabled_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    superseded_by_source_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
    ...
    source_signals: Mapped[list["SourceSignal"]] = relationship(back_populates="source")
